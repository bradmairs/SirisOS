...
Cross-object traversal (e.g. Project → calculation → standard → SirisHydro question → decision) remains future work — v1 was deliberately just the record store and manual-entry UI. Automatic capture from SirisAI conversations shipped later; see "SirisAgent Conversation Memory Suggestions" below. ADR 061.
...
### SirisAgent Conversation Memory Suggestions

Closes the gap Siris Memory's own ADR (061) left open: after each chat reply, a new `SirisMemoryService.suggest()` call asks Ollama to extract up to 3 candidate Facts/Preferences/Observations from the athlete's message, shown as dismissable "Siris noticed: ..." chips under that reply with explicit Save/Dismiss actions -- suggest-then-confirm, chosen over auto-save, so nothing is ever persisted without an explicit tap. Suggestions live only in an in-memory map keyed by chat turn index, deliberately outside the persisted history ADR 100 added -- re-surfacing a suggestion days later, or even just after navigating away from the chat screen and back, would be confusing rather than helpful.

Live testing against the real model surfaced a genuine interaction bug between two existing features. SirisAgent's chat refuses anything that isn't a data question (ADR 091) with a fixed sentence; a personal statement like "I'm a civil engineer and my homelab NAS is named vault" isn't a data question, so Siris correctly refuses it -- but the first version of extraction fed the model *both* turns of the exchange, and the model treated Siris's refusal as proof nothing memory-worthy had been said, 100% reproducible across five repeated real-model calls. Rewording the extraction prompt to explicitly say "ignore Siris's reply, judge only the athlete's message" did not fix it -- still 100% empty at n=5, the same lesson ADR 098 already learned: stacking more instructions onto a small model's prompt doesn't reliably fix a judgment failure, and can make it worse. The fix that actually worked was architectural: stop giving the model Siris's reply at all. `suggest()` now builds its prompt from the athlete's message alone; tested at n=5, the refusal case went from 0/5 to 5/5 correct, with no regression on a clean-reply baseline or a pure data question (3/3 correctly extracting nothing).

A second, unrelated bug only surfaced by clicking the actual Save/Dismiss buttons rather than just checking the chips rendered: the mobile client's `suggest()` built its returned list with `.toList(growable: false)`, so tapping either button threw `UnsupportedError: remove` from the screen's optimistic-removal `setState`, caught silently by Flutter's gesture handler with no visible error. Fixed by making the list growable. End-to-end verified in the browser after both fixes: three suggestion chips rendered from a refusal-triggering message, Save persisted correctly (confirmed via a direct API call and the Memory tab UI), Dismiss removed a chip with no memory record created. Backend: 342 tests pass (10 new service tests, 2 new route tests). ADR 103.
