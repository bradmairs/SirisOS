                ...
                children: [
                  _CalculatorPage(
                    ...
                    calculate: () {
                      ...
                      return [
                        _EngineeringResult('Flow', '${result.flowM3s.toStringAsFixed(3)} m³/s'),
                        _EngineeringResult('Flow', '${(result.flowM3s * 1000).toStringAsFixed(0)} L/s'),
                        _EngineeringResult('Velocity', '${result.velocityMs.toStringAsFixed(2)} m/s'),
                      ];
                    },
                  ),
                  _CalculatorPage(
                    ...
                    calculate: () {
                      ...
                      return [
                        _EngineeringResult('Peak flow', '${result.flowM3s.toStringAsFixed(3)} m³/s'),
                        _EngineeringResult('Peak flow', '${result.flowLs.toStringAsFixed(0)} L/s'),
                      ];
                    },
                  ),
                  _CalculatorPage(
                    ...
                    calculate: () {
                      ...
                      return [
                        _EngineeringResult('Buoyant force', '${result.buoyantForceKn.toStringAsFixed(1)} kN'),
                        _EngineeringResult('Resisting force', '${result.resistingForceKn.toStringAsFixed(1)} kN'),
                        _EngineeringResult('Factor of safety', result.factorOfSafety.toStringAsFixed(2)),
                        _EngineeringResult(
                          'Screening result',
                          result.stable ? 'Resisting > uplift' : 'Uplift exceeds resistance',
                        ),
                      ];
                    },
                  ),
                  _CalculatorPage(
                    ...
                    calculate: () {
                      ...
                      final excess = result.inflowM3s > result.outflowM3s
                          ? result.inflowM3s - result.outflowM3s
                          : 0.0;
                      return [
                        _EngineeringResult('Required storage', '${result.storageM3.toStringAsFixed(1)} m³'),
                        _EngineeringResult('Excess flow', '${(excess * 1000).toStringAsFixed(0)} L/s'),
                      ];
                    },
                  ),
                ],
...
class _CalculatorPage extends StatefulWidget {
  ...
  final List<_EngineeringResult> Function() calculate;
...
class _CalculatorPageState extends State<_CalculatorPage> {
  ...
  void _calculate() {
    setState(() {
      try {
        _results = widget.calculate();
        _error = null;
      } catch (_) {
        _results = null;
        _error = 'Check the entered values and units.';
      }
    });
  }
