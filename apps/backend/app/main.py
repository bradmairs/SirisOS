...
async def host_metrics(_: CurrentUsername) -> HostMetricsResponse:
    ...
    return HostMetricsResponse(
        available=metrics.available, hostname=metrics.hostname, cpu_percent=metrics.cpu_percent,
        memory_percent=metrics.memory_percent, memory_used_bytes=metrics.memory_used_bytes,
        memory_total_bytes=metrics.memory_total_bytes, disk_percent=metrics.disk_percent,
        disk_used_bytes=metrics.disk_used_bytes, disk_total_bytes=metrics.disk_total_bytes,
        load_1m=metrics.load_1m, uptime_seconds=metrics.uptime_seconds,
        generated_at=datetime.now().astimezone().isoformat(), error=metrics.error,
    )
...
async def docker_status(_: CurrentUsername) -> DockerStatusResponse:
    ...
    return DockerStatusResponse(
        available=summary.available, total=summary.total, running=summary.running,
        stopped=summary.stopped, unhealthy=summary.unhealthy,
        containers=[DockerContainerResponse(
            container_id=item.container_id, name=item.name, image=item.image,
            state=item.state, status=item.status, health=item.health,
            cpu_percent=item.cpu_percent, memory_usage_bytes=item.memory_usage_bytes,
            memory_limit_bytes=item.memory_limit_bytes, memory_percent=item.memory_percent,
        ) for item in summary.containers], error=summary.error,
    )
...
async def dashboard(_: CurrentUsername) -> DashboardResponse:
    ...
    return DashboardResponse(
        greeting_name="Brad", homelab=homelab,
