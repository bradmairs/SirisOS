...
- [ ] Strength score — aggregate + per-muscle-group breakdown from historical performance, explicitly framed as personal/relative rather than a universal absolute number -- muscle-group tagging now exists (ADR 083) and can be reused directly
...
- [ ] Run readiness — distinct from general recovery because someone can be HRV-recovered while their legs are wrecked from squats; needs both Health recovery data and recent gym leg volume as inputs. Health recovery data exists (ADR 072/073) and "leg volume" is now queryable via muscle-group tagging (ADR 083); still needs a decision on how to combine the two signals
