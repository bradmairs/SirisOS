...
## Addendum: body shapes and anatomical detail (follow-up)

The initial `BodyDiagram` used primitive rects/ellipses per region, which read as disconnected blocks rather than a body -- fixed by redrawing every shape as hand-traced bezier paths (tapered torso, rounded deltoid caps, arms/legs with real negative space against the torso).

A second pass added anatomical linework (ab segmentation, pec/delt/lat/trap separation, bicep-tricep and quad-hamstring definition, calf lines) drawn as decorative strokes on top of the same six colored regions, so the diagram reads as a detailed muscle-map illustration without implying any data more granular than the six tracked groups -- every decorative line shares its region's single color, nothing is colored independently. Colors also now animate (600ms ease-out) rather than snapping instantly: on first load they fade in from a neutral tone, on a fatigue-data change they cross-fade from the previous value, and on a Front/Back toggle the newly-revealed view's region (chest vs. back) fades in from neutral rather than attempting to morph between the two views' differently-shaped overlay paths.
